...
function postcardBodyFont(mainText: string): { size: number; lh: number } {
  ...
  if (paras <= 8) return { size: 11, lh: 1.5 }; // medium  → +22 %
...
export const ENVELOPE_FONTS = {
  display: "var(--font-display)",
  note: "var(--font-note)",
  label: "var(--font-label)",
} as const;

const B = ENVELOPE_TOKENS;
const F = ENVELOPE_FONTS;
...
export function NavyPatch({ w, h, style }: { w: number; h: number; style?: CSSProperties }) {
...
export function RustPatch({ w, h, style }: { w: number; h: number; style?: CSSProperties }) {
...
export function LavenderBundle({ style }: { style?: CSSProperties }) {
  return (
      ...
      {(
        ...
        ys.map((y, bi) => (
          <ellipse
            ...
            rx="2"
            ry="4"
            fill="oklch(0.62 0.13 280)"
            opacity={0.68 + bi * 0.04}
          />
        )),
      )}
    </svg>
...
export function PressedChrysanthemum({ style }: { style?: CSSProperties }) {
...
export function Tape({ w, h, style }: { w: number; h: number; style?: CSSProperties }) {
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={style} aria-hidden="true">
...
export function PostageStamp({ style }: { style?: CSSProperties }) {
      ...
      {[0, 51, 102, 153, 204, 255, 306].map((deg, i) => {
        ...
        return (
          <ellipse
            ...
            fill={B.accent}
            opacity="0.4"
            transform={`rotate(${deg} ${21 + 7 * Math.cos(r)} ${25 + 7 * Math.sin(r)})`}
          />
        );
      })}
...
export function DenimHeart({ style }: { style?: CSSProperties }) {
  return (
    <svg viewBox="0 0 48 44" style={style} aria-hidden="true">
      ...
      <path
        d="M24 40 L3 20 C3 10 13 6 24 14 C35 6 45 10 45 20 Z"
        fill="none"
        stroke="rgba(220,215,210,0.6)"
        strokeWidth="1.2"
    ...
    >
      <div
        style={{
          position: "absolute",
          inset: "4px",
          border: "0.8px dashed rgba(26,45,74,0.13)",
          borderRadius: "1.5px",
          pointerEvents: "none",
        }}
      />
    ...
    >
      <div
        style={{
          position: "absolute",
          inset: "3.5px",
          border: "1.3px dashed rgba(80,50,10,0.33)",
          borderRadius: "2px",
          pointerEvents: "none",
        }}
      />
      <div
        style={{
          position: "absolute",
          inset: "6px",
          border: "0.7px dashed rgba(80,50,10,0.17)",
          borderRadius: "1px",
          pointerEvents: "none",
        }}
      />
...
export function PostcardSignature({ style }: { style?: CSSProperties }) {
  return (
    <svg
      ...
      style={{
        display: "block",
        filter: "drop-shadow(1.5px 2.5px 3.5px rgba(10,20,50,0.42))",
        ...style,
      }}
      aria-label="MAD"
    >
      ...
      <text
        ...
        fontSize="19"
        letterSpacing="3"
        fill="rgba(242,234,220,0.96)"
      >
        MAD
      </text>
      {/* Script underline */}
      <path
        d="M10,37 Q31,40 52,37"
        fill="none"
        stroke="rgba(195,165,115,0.78)"
        strokeWidth="1.3"
        strokeLinecap="round"
      />
      {/* Heart */}
      <path
        d="M31 50 L23.5 44 C23.5 40.5 27 38.5 31 41.5 C35 38.5 38.5 40.5 38.5 44 Z"
    ...
    >
      <div
        style={{
          position: "absolute",
          inset: "3px",
          border: "1px dashed rgba(180,200,230,0.42)",
          borderRadius: "1px",
          pointerEvents: "none",
        }}
      />
      ...
      <HeartSVG
        style={{ width: 8, height: 8, color: "#d44c30", position: "relative", flexShrink: 0 }}
      />
    </div>
  );
}
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div
            style={{
              ...
              left: "8px",
              right: "8px",
              borderTop: "1px dashed rgba(80,50,10,0.22)",
            }}
          />
        ...
        <DenimHeart
          style={{
            position: "absolute",
            bottom: `${Math.round(ENV_H / 2) + 8}px`,
            left: "50%",
            transform: "translateX(-50%)",
            width: "36px",
            height: "32px",
